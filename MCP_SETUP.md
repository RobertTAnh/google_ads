...
| **Tạo campaign mới** (mutate) | POST | `/mcp/v1/create_campaign` | JSON body: `customer_id`, `campaign_type`, `campaign_name`, `daily_budget`, `keywords` hoặc `keywords_json`, … **SEARCH bidding**: `bidding_strategy` (`MANUAL_CPC` \| `MAXIMIZE_CLICKS` \| `MAXIMIZE_CONVERSIONS` \| `TARGET_CPA`), `max_cpc_ceiling` (trần CPC cho MAXIMIZE_CLICKS), `default_cpc` (chỉ MANUAL_CPC), `target_cpa` (TARGET_CPA) |
...
| **Thêm extension** (mutate, mọi cấp) | POST | `/mcp/v1/add_campaign_extensions` | JSON: `customer_id`, `level` (`customer` \| `campaign` \| `ad_group`), `campaign_id?`, `ad_group_id?`, `sitelinks?`, `callouts?` / `callouts_json?`, `phone_number?`, `mcc_id?` |
