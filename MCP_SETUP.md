...
| **Tạo campaign mới** (mutate) | POST | `/mcp/v1/create_campaign` | JSON body: `customer_id`, `campaign_type`, `campaign_name`, `daily_budget`, … **SEARCH bidding**: `bidding_strategy` (`MANUAL_CPC` \| `MAXIMIZE_CLICKS` \| `MAXIMIZE_CONVERSIONS` \| `TARGET_CPA`), `max_cpc_ceiling` (trần CPC cho MAXIMIZE_CLICKS), `default_cpc` (chỉ MANUAL_CPC), `target_cpa` (TARGET_CPA) |
...
| **Thêm extension campaign** (mutate) | POST | `/mcp/v1/add_campaign_extensions` | JSON: `customer_id`, `campaign_id`, `sitelinks?` `[{link_text, final_url, description1?, description2?}]`, `callouts?` (mảng string), `phone_number?` + `phone_country_code?` (hoặc `call: {phone_number, country_code}`), `mcc_id?` |
