...
def register_mcp_routes(
    ...
    *,
    ...
    @bp.post("/add_keywords")
    def add_keywords_route():
        """Thêm keyword mới vào ad group Search có sẵn."""
        err = _mcp_auth_error_response()
        if err:
            return err
        ...
        if not keywords and body.get("keywords_json"):
            ...
            if isinstance(raw_kw_json, str) and raw_kw_json.strip():
                try:
                    keywords = _parse_keyword_specs(json.loads(raw_kw_json))
                except json.JSONDecodeError as e:
                    return jsonify({"ok": False, "error": f"keywords_json không hợp lệ: {e}"}), 400
            elif isinstance(raw_kw_json, list):
                keywords = _parse_keyword_specs(raw_kw_json)
        ...
        if not keywords:
            return jsonify(
                {
                    "ok": False,
                    "error": "Cần keywords: [{text, match_type?, cpc_bid?}].",
                }
            ), 400

        try:
            client = build_google_ads_client_for_mcc(mcc_id)
            result = add_keywords_to_ad_group(
                client,
                cid,
                ad_group_id,
                keywords,
